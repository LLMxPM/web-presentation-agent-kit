...
只有封面、目录、章节分隔或页面骨架等有明确模板价值、预计跨页/跨项目复用的结构才新建页面组件。指标卡、引用卡、图表和表格只有在职责稳定、会重复且可由 props/slots 表达时才沉淀为内容组件；当前页面独有的叙事包装留在页面源码中。

## 4. 形成固定画布构图

将画布划分为标题区、主体区和辅助区，明确每个主要内容组的空间锚点、宽高、对齐、层级和 overflow 策略。标题可以顶部对齐，但主体面板和稀疏卡片默认应在自己的可用高度内平衡，不要让正文堆在顶部、再用 `mt-auto` 把尾部推到底部。
